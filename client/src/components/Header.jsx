  ...
  return ( 
    <header className='sticky top-0 z-50 inset-x-0 shadow-sm text-white bg-[#121d1c]'>
       <div className='flex justify-between items-center w-full max-w-7xl mx-auto p-3 relative '>
        <Link to='/'>
          <h1 className='  relative my-auto text-md flex flex-wrap items-center'>
            <span className=' font-sans mr-3 font-semibold'>PrimeEstate</span>
          </h1>
        </Link>
         <form
        ...
        >
         ...
         <input 
            ...
            className='flex font-semibold 
             py-1.5 px-6 bg-transparent border-2 border-r-0 text-slate-100 rounded-l-lg focus:outline-none lg:w-52 w-36 text-wrap  flex-wrap text-sm '
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          <button className='p-1.5 pr-3 border border-l-0 border-2 text-slate-100 rounded-r-lg'>
            <SearchIcon className=' font-sans hover:opacity-90 text-slate-100' />
          </button>
          </div>
        </form> 

        <ul className='flex lg:gap-16 gap-2 
        items-center 
        '>
          <Link to='/'>
            <li className='hidden sm:inline  hover:opacity-85 lg:p-4 sm:p-2 font-sans font-semibold '>
              Home
            </li>
          </Link>
          <Link to='/rent'>
